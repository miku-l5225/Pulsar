<!-- src/resources/lorebook/LorebookManager.vue -->
...
import type { LorebookResourceItem } from "./useLorebookResources";
import { useLorebookResources } from "./useLorebookResources";
...
const packageName = computed(() => {
	...
	try {
		...
		console.warn("[LorebookManager] Failed to resolve package name:", e);
		return null;
	}
});

// ==========================
// 使用 Lorebook Resources Hook
// ==========================
const { items, actions } = useLorebookResources(packageName);
...
// 1. 导航/打开
const handleItemClick = async (item: LorebookResourceItem) => {
...
// 4. 重命名
const triggerRename = async (item: LorebookResourceItem) => {
...
// 5. 删除
const triggerDelete = async (item: LorebookResourceItem) => {
	activeMenuId.value = null;
