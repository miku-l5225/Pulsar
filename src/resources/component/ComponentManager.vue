<!-- src/resources/component/ComponentManager.vue -->
...
import type { ComponentResourceItem } from "./useComponentResources";
import { useComponentResources } from "./useComponentResources";
...
const packageName = computed(() => {
	...
	try {
		...
		console.warn("[ComponentManager] Failed to resolve package name:", e);
		return null;
	}
});

// ==========================
// 使用 Component Resources Hook
// ==========================
const { items, actions } = useComponentResources(packageName);
...
// 1. 导航/打开
const handleItemClick = async (item: ComponentResourceItem) => {
...
const handleCreate = async () => {
	const name = prompt("新建组件");
	if (!name) return;

	try {
		await actions.newFile(name + ".vue");
	} catch (e) {
		console.error(e);
		alert("创建失败");
	}
...
// 4. 重命名
const triggerRename = async (item: ComponentResourceItem) => {
...
// 5. 删除
const triggerDelete = async (item: ComponentResourceItem) => {
	activeMenuId.value = null;
