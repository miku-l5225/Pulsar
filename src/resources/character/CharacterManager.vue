<!-- src/resources/character/CharacterManager.vue -->
...
import type { CharacterResourceItem } from "./useCharacterResources";
...
const packageName = computed(() => {
	...
	try {
		...
		console.warn("[CharacterManager] Failed to resolve package name:", e);
		return null;
	}
});

// ==========================
// 使用 Character Resources Hook
// ==========================
const { items, actions } = useCharacterResources(packageName);
...
// 1. 导航/打开
const handleItemClick = async (item: CharacterResourceItem) => {
...
// 4. 重命名
const triggerRename = async (item: CharacterResourceItem) => {
...
// 5. 删除
const triggerDelete = async (item: CharacterResourceItem) => {
	activeMenuId.value = null;
