<!-- src/resources/background/BackgroundManager.vue -->
...
import type { BackgroundResourceItem } from "./useBackgroundResources";
import { useBackgroundResources } from "./useBackgroundResources";
...
const packageName = computed(() => {
	...
	try {
		...
		console.warn("[BackgroundManager] Failed to resolve package name:", e);
		return null;
	}
});

// ==========================
// 使用 Background Resources Hook
// ==========================
const { items, actions } = useBackgroundResources(packageName);
...
// 1. 导航/打开
const handleItemClick = async (item: BackgroundResourceItem) => {
...
// 4. 重命名
const triggerRename = async (item: BackgroundResourceItem) => {
...
// 5. 删除
const triggerDelete = async (item: BackgroundResourceItem) => {
	activeMenuId.value = null;
      ...
      >
        ...
        <div class="flex-1 min-w-0 flex flex-col">
          <div class="flex items-center gap-1.5">
            ...
            <ImageIcon v-else class="w-3.5 h-3.5 text-muted-foreground" />
