// src/composables/useExecuteContext.ts

import { push } from "notivue";
import { embed, generateText, streamText } from "@/ai";
import type { ApiReadyMessage } from "@/resources/chat/chat.types";
import type { EnhancedApiReadyContext } from "@/resources/chat/EnhancedApiReadyContext/EnhancedApiReadyContext";
import { ModelConfigDefinition } from "@/resources/modelConfig/modelConfig";
import { separatePrompts } from "@/resources/preset/preset";
import { SemanticTypeMap } from "@/resources/SemanticType";
import { SettingDefinition } from "@/resources/setting/setting";
import type {
  ExecuteContext,
  fromUseExecuteContext,
  RegexRule,
} from "@/resources/shared.types";

// 定义资源快照类型
export interface ResourceSnapshot {
  character?: Array<{ path: string; content: any }>;
  preset?: Array<{ path: string; content: any }>;
  lorebook?: Array<{ path: string; content: any }>;
}
  ...
  if (resourceSnapshot) {
    for (const [type, resources] of Object.entries(resourceSnapshot)) {
      ...
      if (definition && "wrapperFunction" in definition) {
        // @ts-expect-error - 这里的类型推断可能比较复杂，假设 wrapperFunction 能够处理
